...
Now, when we multiply this matrix by a vector ( v = [x, y] ), we’re expressing ( v ) in this new basis:

$M \cdot v = x \cdot [2,1] + y \cdot [1,3]$

So if ( v = [2, 3] ), then:

$M \cdot v = 2[2,1] + 3[1,3] = [7,11]$

Now, if we wanted to multiply this matrix by an input vector, $\mathbf{v} = [x,y] = [2,3]$, we can just take each column of the matrix and multiply it by the element in the same index in the vector. Let's walk through it step-by-step:
...
This operation is also called the **Dot Product**. I
...
What you end up getting is an output that is a weighted combination of the inputs.
...
So, if (A) is 2×3 and (B) is 3×2, the (i, j)th entry of (A \times B) is:

$$
[
(A \times B)*{ij} = \text{dot}(A*{i,*}, B_{*,j})
]
$$
...
- The **dot product** measures how aligned two vectors are — it’s a single scalar.
...
- Dot product → one projection (single output)
- Matrix multiplication → many projections at once (multi-dimensional transformation)

---

Would you like me to show a **visual** of how one matrix multiplies another using the dot-product view — with arrows or grids showing each row·column interaction? It’s very helpful for intuition, especially in the context of attention and LLMs.
