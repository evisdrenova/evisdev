...
**Note:** Why so many dimensions? Because language is complex! A word might need dimensions for:
...
4. The solid orange vector is the 2st column of the matrix $M = \begin{bmatrix} 2 & 1 \\ 1 & 3 \end{bmatrix}$, which is $\begin{bmatrix} 1 \\ 3 \end{bmatrix}$53.
...
Imagine we have simple 2D "word vectors" where each element in the vector represents some characteristic of the word. For example:

- "king" → `[0.9, 0.1]` (high masculinity, low femininity)
- "queen" → `[0.9, 0.9]` (high masculinity/royalty, high femininity)
- "man" → `[0.8, 0.0]` (masculine, neutral)
- "woman" → `[0.0, 0.9]` (neutral, feminine)

Now if we do the vector arithmetic: `king - man + woman`:

$$
[0.9, 0.1] - [0.8, 0.0] + [0.0, 0.9] = [0.1, 1.0]
$$

This result is close to "queen"! We've done the same weighted sum operation,
except now it's operating on **semantic meaning** rather than spatial directions.

Let's jump into neural networks.
