...
Like with many of my blogs, I start at the beginning and then build to the intuition.
...
In our simple neural net, we have 3 layers:

- **Input layer:** 3 inputs → $x_1, x_2, x_3$
- **Hidden layer:** 3 hidden units → $h_1, h_2, h_3$
- **Output layer:** 1 output → $\bar{y}$

Each connection (represented as lines) between nodes corresponds to a **weight** in a matrix. Since we have 3 input nodes feeding into 3 hidden nodes, we can create a 3×3 **weight matrix**:
...
In practice, we would randomly initialize these weights before we train the neural net. So it might look something like:
...
In order to calculate the hidden layer, we can represent our input layer $x_1, x_2, x_3$ as the vector $[x_1, x_2, x_3]$.

To calculate our hidden layer, we multiply our input vector with our weight matrix:

- Each **row** corresponds to an input ($x_1, x_2, x_3$)
- Each **column** corresponds to a hidden unit ($h_1, h_2, h_3$)
...
## Step 1: Input → Hidden Layer Weights

Since you have **3 inputs feeding into 3 hidden units**, you need a **weight matrix** of shape $W^{(1)} \in \mathbb{R}^{3 \times 3}$ (3 rows for inputs × 3 columns for hidden units).
...
## Step 2: Hidden → Output Layer Weights

Now we have **3 hidden units feeding into 1 output**. That means a weight matrix of shape $W^{(2)} \in \mathbb{R}^{3 \times 1}$.
...
The entire forward pass is just **two matmuls in sequence**:

$$
\bar{y} = \left( [x_1, x_2, x_3] \cdot W^{(1)} \right) \cdot W^{(2)}
$$

That's the core of what every layer in a neural net does:

- First **linear combination of inputs → hidden layer**
- Then another **linear combination of hidden layer → output**

Add nonlinearities (like ReLU, GELU, softmax) between these matmuls and you get the expressive power of neural networks.

## Why It Matters for LLMs

When LLMs apply a weight matrix to an embedding vector:

- They're transforming the input representation into a new **feature space**.
- Each layer tweaks the orientation, scaling, and mixing of features so that relationships (like "cat" and "dog" being semantically close) become geometrically encoded.
- The self-attention operation itself is just a specialized sequence of matmuls that project vectors into Query, Key, and Value spaces, then combine them.
