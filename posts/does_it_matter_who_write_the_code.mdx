...
In an ideal world, I don't think it matters who writes the code because the code would always be right. The problem is that we don't live in an ideal world. AI often generates code that doesn't work and is unnecessarily verbose. It riddles the codebase with unnecessary comments which makes it difficult to stay focused on the code that matters.
...
With that being said, I could see the need for organizations to watermark their code to understand who wrote it. Just like anything else, it's important to understand the risk vectors of your business and potentially buggy and insecure code is certainly one of those.

I'm sure in the near future someone will create git blame for AIs. With a direct IDE integration it would theoretically be straight forward. Whenever an AI inserts code into a codebase, update the metadata of those lines to indicate that an AI was the writer. Coincidentally, I saw that [Zed today announced](https://zed.dev/blog/sequoia-backs-zed) that they're working on a CRDT driven database to store code changes along with conversation history.
