...
So now that we can measure how right or wrong our predictions are, how do we improve our model to get better over time?

# Hill Climbing

One way to think about loss functions and improving the accuracy of your model is to imagine your loss function as a landscape - a hilly terrain where the height at any point represents how wrong (or right) your model is with those particular weights. The lower the point is in the landscape, the lower the loss and the better the model.
...
When we first initialize our model, we do it with random weights. (We have to start somewhere.) As we start to train our model we might be "dropped" anywhere in this landscape. Every step we take changes the landscape across the millions or billions of parameters that we have. So we need to find a way to climb the hills and valleys in order to get to the lowest point in the landscape. In reality, we don't always reach the global minimum. We might find a local minimum and that might be good enough.
...
Generally speaking this works well but practically, it has some problems.
...
The good thing is that we can address of each of these problems by making some slight changes to the gradient descent formula.
...
It turns out, it doesn't impact the model all that much but matkes it significantly faster to run. Let's see how this works.
