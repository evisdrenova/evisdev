...
Here's how the two passes work together:

**Forward Pass (left to right):**

- Take your training examples
- Push them through the model layer by layer
- Get predictions at the end
- Compute the loss (how wrong you were)

**Backward Pass (right to left):**

- Start with the loss
- Work backwards through the model using the chain rule
- Calculate ∂Loss/∂(each parameter) - the gradient for every parameter
...
1. Forward pass: Make predictions, compute loss
2. Backward pass: Compute gradients, update parameters
3. Repeat with next batch of data
   Specifically, we:

4. Process all training examples. If we have 1 trillion tokens in our pre-training data set, then we process 1 trillion tokens through the model.
5. Compute the loss on the 1 trillion tokens that we processed.
6. Update all of the parameters in the model.
7. Take one step down the hill.

If you're thinking, "wow that's a lot to process for just one step", then you're exactly right. It is. And it's not the only problem.
...
When we train a model on a forward pass, we process all of the training examples in our data. That forward pass updates al

Well what if we didn't do the entire dataset? What if we just did parts of it, how does that impact the quality of our model?

It turns out, it doesn't really impact the model all that much and makes it significantly faster to run. Let's see how this works.

Instead of processing all of the 1 trillion tokens in our pre-training data set, what if we just did a small batch?
...
Remember our blindfolded hiker trying to reach the valley? In basic gradient descent, before taking each step, they carefully feel the slope of the _entire mountain_ beneath them—every rock, every crevice—to determine the exact direction downward.

But what if you're on a massive mountain? Feeling every square inch before each step would take forever. You'd be so slow that by the time you finish measuring, the sun would set!

Here's a clever insight: what if instead of measuring the entire mountain, you just measured the slope of the ground right around where you're standing? Sure, it won't be as precise as measuring everything, but it'll be **much, much faster**. And if the patch you're measuring is reasonably representative of the overall terrain, you'll still head in roughly the right direction.
...
Yes, each individual measurement is noisier—like how your foot might land on a random rock that doesn't represent the overall slope. But here's the magic: you can take _many more steps_ in the same amount of time.
...
The noise actually _helps_! Here's why:

Imagine you're walking down the mountain and you stumble into a small pothole. With perfect gradient descent, you might get stuck there thinking "I'm at the bottom!" But with noisy SGD, the randomness from sampling different mini-batches can jostle you out of that pothole and help you continue toward the true valley.

This noise acts as a form of **regularization** and helps the model generalize better to new data.
