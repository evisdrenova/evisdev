...
This makes intuitive sense. We're asking the model to predict 5 different tokens. For each token, we see how right or wrong it was (our loss) and then we average that to get a final loss number. If the number is high then our model was often wrong. If it's low, then our model was often right. All relative to our training data, of course.

So now that we can measure how right or wrong our predictions are, how do we improve our model to get more accurate and precise over time?

<Note>

You might have noticed that I used the words "accuracy" and "precise" a few times. In machine learning, they actually have different meanings.

- **Accuracy** is the overall correctness of all predictions (what fraction of all predictions were correct).
- **Precision** measures, of all the cases where the model predicted positive, how many were actually positive.

There is actually a third measure that I didn't mention called **recall**.

- **Recall** measures, of all the actual positive cases, how many did the model correctly identify.

These might all sound the same but they are nuanced and have important differences. You can read more on accuracy vs. precision vs. recall [here.](https://developers.google.com/machine-learning/crash-course/classification/accuracy-precision-recall)
...
For simple models, this landscape might be a smooth bowl—there's one obvious lowest point. But for LLMs with billions of parameters? The landscape is extraordinarily complex, with countless hills, valleys, and saddle points in billions of dimensions.
