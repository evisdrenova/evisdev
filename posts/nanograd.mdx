...
```python
# Leaf tensor (input or parameter)
x = Tensor([1.0, 2.0, 3.0], requires_grad=True)
# x.grad_fn = None (it's a leaf)
# x.grad = None (not computed yet)

# Intermediate tensor (result of operation)
y = x * 2
# y.grad_fn = MulBackward (stores how to compute derivative of multiplication)
# y.parents = [x] (links back to input)
# y.requires_grad = True (inherited from x)
```
...
[To be written - covers how tensors track their computational history]

## 3. Gradient Functions

[To be written - covers how each operation stores its derivative computation]
