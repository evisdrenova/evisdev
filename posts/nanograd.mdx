...
The neuron will have a `weights` field that is a vector of Tensors and a `bias` field which is just a tensor.
...
The forward pass function is a straight implementation of `output = relu(w₁×x₁ + w₂×x₂ + ... + wₙ×xₙ + b)`. We initialize a new Tensor with data set to 0.0, then iterate over the weights and input multiplying them together at the same indices and then accumulating the sum. We add the bias and then call the `relu()` method.
...
First, we need a dataset. Here is a pretty basic one for us to work with:
...
2. For each data point:
   ...
   - Backward pass
3. Update weights (using the update function we created earlier)
